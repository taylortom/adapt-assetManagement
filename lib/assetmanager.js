...
var path = require('path'),
    ...
    OutputConstants = require('./outputmanager').Constants;

// count to guard against duplicate asset filenames
var assetCount = 0;
...
exports = module.exports = {
  ...
  createAsset: function (data, options, next) {
    ...
    database.getDatabase(function (error, db) {
      ...
      // also set the updated date, otherwise we get odd search results
      if (!data.updatedAt) {
  ...
  postAsset: function (req, res, next) {
    ...
    form.parse(req, function (error, fields, files) {
      ...
      filestorage.getStorage(repository, function (error, storage) {
        ...
        var user = usermanager.getCurrentUser();
        var file = files.file;
        var fileData = self.generateFileLocationData(file);
        var fileOptions = {
          createMetadata: true,
          createThumbnail: true,
          thumbnailOptions: {
            width: self.Constants.THUMBNAIL_WIDTH,
            height: self.Constants.THUMBNAIL_HEIGHT
          }
        };
        // the repository should move the file to a suitable location
        storage.processFileUpload(file, fileData.path, fileOptions, function (error, storedFile) {
          if (error) {
            return next(error);
          }
          ...
          // add the following to storedFile
          var asset = _.extend({
            title: fields.title,
            description: fields.description,
            repository: repository,
            filename: fileData.name,
            directory: fileData.dir,
            isDirectory: false,
            isRestricted: fields.isRestricted,
            tags: (fields.tags && fields.tags.length) ? fields.tags.split(',') : [],
            workspaces: (fields.workspaces) ? JSON.parse(fields.workspaces) : {},
            createdBy: user._id
          }, storedFile);
  ...
  putAsset: function (req, res, next) {
    ...
    async.waterfall([
      function getData(cb) {
        if(!_.isEmpty(req.body)) formData = req.body;
        else if(!_.isEmpty(req.query)) formData = req.query;
        // not multipart
        if(formData && !formData.file) return cb();
        // handle multipart
        var form = new IncomingForm();
        form.parse(req, function (error, fields, files) {
      ...
      function getAssetDocument(cb) {
        self.retrieveAsset({ _id: req.params.id }, function(error, assetDocs) {
          if (!assetDocs || assetDocs.length !== 1) {
            return cb(new Error('asset not found'), { status: 404, success: false });
          }
          assetDoc = assetDocs[0];
          cb();
        });
      },
      function getAssetDelta(cb) {
        delta = _.extend(assetDoc, {
          title: formData.title || assetDoc.title,
          description: formData.description || assetDoc.description,
          isRestricted: formData.isRestricted || assetDoc.isRestricted,
          workspaces: (formData.workspaces && JSON.parse(formData.workspaces) || {}) || assetDoc.workspaces,
          tags: (formData.tags && formData.tags.split(',') || []) || assetDoc.tags,
        });
        cb();
      },
      // if file
      function storeFile(cb) {
        ...
        filestorage.getStorage(assetDoc.repository, function (error, storage) {
          if(error) return cb(error);
          var fileData = self.generateFileLocationData(file, assetDoc);
          storage.getFileStats(fileData.path, function(error, stats) {
            if(error && error.code !== 'ENOENT') return cb(error);
            // TODO exact copy of options in postAsset, should be stored somewhere?
            var fileOptions = {
              createMetadata: true,
              createThumbnail: true,
              thumbnailOptions: {
            ...
            storage.processFileUpload(file, fileData.path, fileOptions, function (error, storedFile) {
              if (error) return cb(error);
              delta = _.extend(delta, storedFile, {
                filename: fileData.name,
                directory: fileData.dir,
                isDirectory: false
              });
      ...
      // /if file
      function updateAsset(cb) {
        self.updateAsset({ _id: assetDoc._id }, delta, function (error, assetRec) {
          if (error) return cb(error);
          return cb(null, { status: 200, success: true });
        });
      }
    ], function(error, data) {
      if(error) {
  ...
  serveAsset: function (req, res, next) {
    ...
    this.retrieveAsset({ _id:  assetId}, function (error, assetRecs) {
      ...
      filestorage.getStorage(assetRec.repository, function (error, storage) {
        ...
        storage.createReadStream(assetRec.path, { bufferSize: self.Constants.STREAM_BUFFER_SIZE }, function (stream) {
          res.writeHead(200, {
    ...
    this.retrieveAsset({ _id: req.params.id }, function (error, assetRecs) {
      ...
      if (assetRec.thumbnailPath && assetRec.thumbnailPath !== 'false') {
        filestorage.getStorage(assetRec.repository, function (error, storage) {
          ...
          storage.createReadStream(assetRec.thumbnailPath, { bufferSize: self.Constants.STREAM_BUFFER_SIZE }, function (stream) {
        ...
        var img = new Buffer(self.Constants.DEFAULT_THUMBNAIL_BASE64, 'base64');
    ...
    database.getDatabase(function (error, db) {
      ...
      db.retrieve('asset', { _id: req.params.id }, options, function (error, assetRecs) {
        ...
        filestorage.getStorage(assetRec.repository, function (error, storage) {
          ...
          storage.createReadStream(assetRec.path, { bufferSize: self.Constants.STREAM_BUFFER_SIZE, forceMaster: true }, function (stream) {
            res.writeHead(200, {
            'Content-Type': assetRec.mimeType,
            'Content-Length': assetRec.size
          });
  ...
  generateFileLocationData: function(newFile, existingFile) {
    var ext = path.extname(newFile.name);
    var name = (existingFile) ? existingFile.filename : Date.now() + '_' + (assetCount++) + ext;
    var dir = OutputConstants.Folders.Assets;
    return {
      name: name,
      ext: ext,
      dir: dir,
